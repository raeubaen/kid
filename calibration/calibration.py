...
fig5, ax3 = plt.subplots(figsize=(20, 10))
...
for i in range(len(cycles)):
  ...
  binned_data, bins, _ = ax3.hist(temp, bins=n_bins, label=f'{cycles[i]} cycles', histtype='step', range = (x_min, x_max), color=colors[i])
  ...
  out = fit(gauss, y = binned_data, x = center_bins, sy = np.sqrt(binned_data+1), sx = 0.0001,
  par0 = [amp, mean, sigma], par_names=['amp', 'mean', 'sigma'],
  ax1=ax3, ax2=axs1[i], xlabel=r"Phase (mrad)", ylabel="Counts",
  title=r'Histogram', color=colors[i])

  axs0[i].set_ylim(0, 600)
  axs0[i].legend(prop={'size': 16})
