...
// Arduino due processor datasheet https://ww1.microchip.com/downloads/en/DeviceDoc/Atmel-11057-32-bit-Cortex-M3-Microcontroller-SAM3X-SAM3A_Datasheet.pdf
...
int trigger(Sample *s){
    if (s->I > 1290){ // just a reasonable number
      return 1;
    }
    else return 0;
}
...
void setup()
{
  ...
  ADC->ADC_MR |= 1 << 7 | 1 << 6; // set free running mode (page 1333 of the Sam3X datasheet)

  // see pinout for the mapping between Arduino pins and ADC channels
  ...
  ADC -> ADC_CHER = ADC_CHANNELS;   // enable channels  (see page 1338 of datasheet )
  ADC->ADC_CR = 1 << 1;       // begin ADC conversion
...
void loop() {
  if (SerialUSB.available() > 0) // polls whether anything is ready on the read buffer - nothing happens until there's something there
  {
    ...
    while (millis() < (start_time + acquisition_time))
    {
      ...
      s->I = ADC->ADC_CDR[I_CHANNEL_NUM];                        // read value A0
      s->Q = ADC->ADC_CDR[Q_CHANNEL_NUM];
      s->t = micros();
      if (trigger(s)){
