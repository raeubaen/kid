...
int i, ADC_CHANNELS, I_CHANNEL_NUM, Q_CHANNEL_NUM, THRESHOLD;  // THRESHOLD is the RMS of noises
...
int trigger(Sample *s, Sample *r) {
    if (abs(s->I - r->I) > THRESHOLD * time_interval){ 
      return 1;
    }
    else return 0;
}
...
void loop() {
  ...
  if (SerialUSB.available() > 0) {
    ...
    while (millis() < end_millis) {
       ...
       */
      s = acquire_data();
      r = &(circ_buffer->buffer[circ_buffer->end - 1]);
