...
for i in tqdm(range(10000)): #in teoria tutto
    ...
    if df.phase.rolling(50).mean().diff(periods=50).max() < 5:
...
np.savetxt("noise_phase_peaks_lunga.txt", phase_peaks)

to_plot = random.choices(cut, k=100)
n = 10
fig, axs = plt.subplots(n, n, figsize=(50, 50), sharex=True, sharey=True)
for i in range(n):
  for j in range(n):
    df = to_plot[n*i+j]
    axs[i][j].plot((df.t - df.t.iloc[0])/1000, df.phase.iloc[:200].mean() - df.phase)
    if j==0:
      axs[i][j].set(ylabel="mrad")
    if i==n-1:
      axs[i][j].set(xlabel="ms")
plt.show()

plt.hist(phase_peaks, bins=200)
plt.show()
