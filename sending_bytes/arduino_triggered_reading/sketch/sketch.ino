...
Sample eot; // End of Transmission signal

CircBuffer *circ_buffer;
Sample *storage, *to_write_samples; // pointer to the first element of arrays of samples
...
void send_data_to_serial(CircBuffer *c, Sample *s) {
  ...
  for(j=0; j<BUFFER_SIZE; j++) {
    olds = pop(c);
    if (olds == NULL) break;
    s[BUFFER_SIZE - active + j] = *olds;
  }
  // tramsmitted data are:
  // 'active' elements from circ_buffer, 'STORAGE_SIZE' elements read after trigger + '1' eot
  SerialUSB.flush();
  SerialUSB.write((byte*)&(s[BUFFER_SIZE - active]), sizeof(Sample)*(active + STORAGE_SIZE + 1));
  SerialUSB.flush();
}
...
void setup() {
  ...
  // last element of the array is reserved for End of Transmission signal (0, 0, 0 struct)
  to_write_samples = (Sample*) malloc(sizeof(Sample)*(BUFFER_SIZE + STORAGE_SIZE + 1));

  eot.I = 0;
  eot.Q = 0;
  eot.t = (unsigned int) 0;
  to_write_samples[BUFFER_SIZE + STORAGE_SIZE] = eot;
  // the buffer is put before the storage, as this is the order they must have when writing to serial
  storage = &(to_write_samples[BUFFER_SIZE]);
}
